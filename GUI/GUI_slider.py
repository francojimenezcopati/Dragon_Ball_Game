...
class Slider(Widget):
    def __init__(
        ...
        self.rectangulo_circulo.center = (
            x + w * value,
            self.slave_rect.centery,
        )
    ...
    def update(self, lista_eventos):
        ...
        if mouse_buttons[0]:
            ...
            if self.slave_rect_collide.collidepoint(mouse_pos):
                valor = (
                    mouse_pos[0] - self.slave_rect_collide.x
                ) / self._slave.get_width()

                self.value = round(valor * 100) / 100  # redondeo el valor

                self.rectangulo_circulo.center = (
                    self.slave_rect.x + self._slave.get_width() * self.value,
                    self.slave_rect.centery,
                )
        self.draw()
