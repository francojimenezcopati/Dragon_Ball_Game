...
class Enemigo(Personaje):
    def __init__(self, velocidad, pos, vida, ataque, tipo, jefe=False):
        ...
        if not jefe:
            ...
            # Muerte
            self.velocidad_animacion_muerte = (
                VELOCIDAD_ANIMACION_MUERTE_ENEMIGOS
            )
            ...
            self.rayo_cargando_rect = self.rayo_cargando.get_rect(
                midright=(49 * TILE_SIZE, 179)
            )
            ...
            self.rayo = pygame.transform.scale(
                self.rayo,
                (self.rayo.get_width() * 2.367, self.rayo.get_height()),
            )
            self.rayo_largo_rect = self.rayo.get_rect(
                midright=(49 * TILE_SIZE, 179)
            )
    ...
    def muerte(
        self, dt
    ):  # las animaciones de muerte ya estan incluidas en el diccionario de animaciones de cada robot, estado = 'muerte' + sentido
        self.direccion.x = 0

        self.frame_index += self.velocidad_animacion_muerte * dt
        if self.frame_index >= len(
            self.animaciones[self.estado + self.sentido]
        ):
            self.frame_index = (
                len(self.animaciones[self.estado + self.sentido]) - 1
            )  # se queda en el ultimo frame
        self.image = self.animaciones[self.estado + self.sentido][
    ...
    def apuntar_jugador(self, vec_jugador):
        ...
        vector_diferencia = (
            vec_jugador - vec_enemigo
        )  # positivo-> derecha, negativo-> izq
    ...
    def update(
        ...
        if self.vida <= 0:  # muerto
            ...
            if (
                self.dispara and self.tiempo_disparar > TIEMPO_DISPARO_ENEMIGO
            ):  # tiempo entre rafagas
                self.estado = "ataque"

                if (
                    self.espera_entre_disparos > ESPERA_ENTRE_DISPAROS
                ):  # tiempo entre disparos de la rafaga
                    self.espera_entre_disparos = 0
                    self.disparar()
                    self.disparos_rafaga += 1
                    if (
                        self.disparos_rafaga >= DISPAROS_ENEMIGOS
                    ):  # cantidad de disparos en la rafaga
                        self.disparos_rafaga = 0
                        self.tiempo_disparar = 0
    ...
    def animar_jefe(self, dt, frames, pantalla):
        ...
        if self.frame_index >= len(frames):  # -> frame de disparo
            ...
            if self.tiempo_transcurrido_frame < self.tiempo_espera_disparo:
                ...
                self.tiempo_transcurrido_frame
                < self.tiempo_disparo + self.tiempo_espera_disparo
            ):
